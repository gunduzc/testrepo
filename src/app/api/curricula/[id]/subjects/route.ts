 ...
 * POST /api/curricula/[id]/subjects - Add subject to curriculum
 */
...
export async function POST(
  ...
  try {
    ...
    if (!["ADMIN", "EDUCATOR"].includes(session.user.role)) {
      return NextResponse.json(
        { error: { code: "FORBIDDEN", message: "Only educators can add subjects" } },
        { status: 403 }
      );
    }
    ...
    const subject = await curriculumService.addSubject(id, data, session.user.id);
